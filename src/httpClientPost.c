...
// Declaración de la variable global definida externamente
...
esp_err_t sendJsonPost(const char *url) {
  ...
  if (jsonOutput == NULL) {
    ESP_LOGE(TAG, "jsonOutput es NULL, no se puede enviar");
  ...
  esp_http_client_set_post_field(client, jsonOutput, strlen(jsonOutput));
